...
import { getInvoiceByToken, seedIfEmpty } from "@/app/lib/mock-store";
        ...
        <div className="mx-auto flex max-w-2xl flex-col gap-4 rounded-[1.875rem] bg-white px-6 py-10 text-center shadow-sm sm:px-8">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm leading-relaxed text-copy sm:text-base">
              This invoice link may be invalid, expired, or not available in the
              current demo session yet.
            </p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
            <Link
              href="/"
              className="inline-flex min-h-12 items-center justify-center rounded-full bg-primary px-6 text-sm font-semibold text-white transition-colors hover:bg-primary/90"
            >
              Go to homepage
            </Link>
            <a
...
export default function PublicInvoiceClient({ token }: { token: string }) {
  ...
  const invoice = useSyncExternalStore<Invoice | null | undefined>(
    subscribe,
    () => getInvoiceByToken(token) ?? null,
    () => undefined,
  );
        ...
        <div className="mx-auto flex max-w-3xl flex-col gap-6">
          <div className="space-y-2">
            ...
            <p className="max-w-2xl text-sm leading-relaxed text-copy sm:text-base">
              Review the charges for your inspection and delivery request before
          ...
          <div className="overflow-hidden rounded-4xl bg-white shadow-sm">
                ...
                <div className="flex flex-col items-start gap-2 sm:items-end">
                  <span
                    className={`inline-flex min-h-10 items-center gap-2 rounded-full border px-4 text-sm font-bold ${
                      isPaid
                        ? "border-lime/30 bg-lime/20 text-lime"
                        : "border-orange-400/30 bg-orange-400/20 text-orange-200"
                    }`}
                  >
                    <span
                      className={`h-2 w-2 rounded-full ${isPaid ? "bg-lime" : "bg-orange-300"}`}
                    />
                    {isPaid ? "PAID" : "UNPAID"}
                  </span>
            ...
            <div className="grid gap-5 border-b border-surface-alt px-5 py-5 sm:grid-cols-2 sm:px-8">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <p className="text-sm text-copy">
                    Inspection and delivery coordination by BuyPadi.ng
                  </p>
            ...
            <div className="px-5 py-6 sm:px-8">
              <div className="space-y-3 sm:hidden">
                ...
                <div className="rounded-2xl bg-surface px-4 py-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="font-semibold text-heading">Delivery Fee</p>
                      <p className="mt-1 text-xs text-muted">
            ...
            <div className="px-5 py-6 sm:px-8 sm:pb-8">
              {!isPaid ? (
                <>
                  {/* TODO: Replace href with real payment gateway URL when backend wiring starts. */}
                  <button
                    type="button"
                    disabled
                    className="inline-flex min-h-14 w-full items-center justify-center gap-3 rounded-2xl bg-lime px-6 text-center font-display text-lg font-bold text-white opacity-75 shadow-md"
                    title="Payment gateway coming soon"
                  >
                    Pay {formatNaira(invoice.total)}
                    <svg
                      className="h-5 w-5"
                      fill="none"
                      viewBox="0 0 24 24"
                  ...
                  <p className="mt-3 text-center text-xs text-muted">
                    Secure payment link will appear here once the gateway is
                    connected.
                  </p>
                </>
              ) : (
          ...
          <div className="rounded-[1.75rem] bg-white px-5 py-5 shadow-sm sm:px-6">
            ...
            <p className="mt-2 text-sm leading-relaxed text-copy">
              If you have any questions about this invoice, reach out to BuyPadi
              support and quote{" "}
